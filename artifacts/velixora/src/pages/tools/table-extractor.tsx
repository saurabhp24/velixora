...
export default function TableExtractor() {
  ...
  const handleProcess = async (f: File) => {
    ...
    try {
      ...
      const csvData = await openaiChat(
        key,
        [
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto">
      <header className="mb-10 text-center">
        <h1 className="text-4xl font-bold tracking-tight mb-2">Table <span className="bg-clip-text text-transparent gradient-violet-cyan">Extractor</span></h1>
