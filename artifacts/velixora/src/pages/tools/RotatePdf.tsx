      ...
      <AnimatePresence mode="wait">
        {result ? (
          ...
          <motion.div key="editor" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              <div className="flex gap-2 mb-5">
                {([90, 180, 270] as Rotation[]).map((deg) => (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white/60 hover:text-white hover:bg-white/10 border border-white/10 transition-all"
                    data-testid={`rotate-all-${deg}`}
                  >
                    <RotateCw className="w-3 h-3" /> All {deg}°
                  </button>
                ))}
                <button
                  onClick={() => setRotations(Array(pages.length).fill(0))}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white/60 hover:text-white hover:bg-white/10 border border-white/10 transition-all ml-auto"
                  data-testid="reset-rotations-btn"
                >
                  Reset all
                </button>
              </div>
            ...
            {/* Page grid */}
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3" data-testid="rotate-page-grid">
              {pages.map((page, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="px-2 py-1.5 flex items-center justify-between">
                      ...
                      <span className={`text-xs font-bold ${rot !== 0 ? "text-primary" : "text-white/30"}`}>
                        {rot}°
                      </span>
                    </div>
                    {/* Hover overlay */}
