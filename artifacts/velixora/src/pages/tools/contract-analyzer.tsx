  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto">
      <header className="mb-10 text-center">
        <h1 className="text-4xl font-bold tracking-tight mb-2">Contract <span className="bg-clip-text text-transparent gradient-violet-cyan">Analyzer</span></h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             <div className="glass p-6 rounded-xl border border-destructive/20 bg-destructive/5">
                <h3 className="text-lg font-semibold text-destructive mb-4 flex items-center gap-2">
                  <ShieldAlert className="w-5 h-5" /> Potential Risks
                </h3>
             ...
             <div className="glass p-6 rounded-xl border border-primary/20 bg-primary/5">
                <h3 className="text-lg font-semibold text-primary mb-4 flex items-center gap-2">
                  <ShieldCheck className="w-5 h-5" /> Key Clauses
                </h3>
                <ul className="space-y-3">
                  {analysis.keyClauses?.map((c:string, i:number) => (
                    <li key={i} className="text-sm text-foreground flex items-start gap-2">
