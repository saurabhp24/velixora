...
function PageThumbnail({
  ...
  return (
    <Reorder.Item
      ...
      whileDrag={{ scale: 1.05, zIndex: 50, boxShadow: "0 20px 60px rgba(79,110,247,0.4)" }}
      layout
      transition={{ duration: 0.2 }}
    >
      <motion.div
        className="group relative rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm overflow-hidden cursor-default"
        style={{ boxShadow: "0 4px 20px rgba(0,0,0,0.3)" }}
        whileHover={{ borderColor: "rgba(79,110,247,0.5)", boxShadow: "0 8px 30px rgba(79,110,247,0.2)" }}
        transition={{ duration: 0.2 }}
      >
        {/* Drag handle */}
        <div
          className="absolute top-2 left-2 z-10 p-1.5 rounded-lg bg-black/40 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity cursor-grab active:cursor-grabbing"
        ...
        <div className="absolute top-2 right-2 z-10 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            ...
            className="p-1.5 rounded-lg bg-black/40 backdrop-blur-sm hover:bg-blue-500/40 transition-colors"
            title="Rotate 90°"
          ...
          <button
            ...
            className="p-1.5 rounded-lg bg-black/40 backdrop-blur-sm hover:bg-red-500/40 transition-colors"
            title="Remove page"
...
export default function RearrangePages() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      setResult({
        blob,
        name: file.name.replace(".pdf", "-rearranged.pdf"),
      });
      ...
      toast({
        ...
        description: "Could not rearrange pages. Please try again.",
        variant: "destructive",
      });
    } finally {
      ...
      <AnimatePresence mode="wait">
        {result ? (
          ...
          >
            ...
            {/* Instructions bar */}
            <div
              className="flex items-center gap-2 mb-6 px-4 py-3 rounded-xl text-sm text-white/50"
              ...
              <div className="flex items-center justify-between text-xs text-muted-foreground px-1">
                ...
                {pages.length > 0 && (
                  <span className="text-primary/70">
                    Order: {pages.map((p) => p.originalIndex + 1).join(" → ")}
                  </span>
                )}
              </div>
