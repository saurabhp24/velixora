...
export default function Compress() {
  ...
  const handleQualityChange = (vals: number[]) => {
    setQuality(vals[0]);
    if (file) processImage(file, vals[0]);
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto">
      <header className="mb-10 text-center">
        <h1 className="text-4xl font-bold tracking-tight mb-2">Smart <span className="bg-clip-text text-transparent gradient-violet-cyan">Compressor</span></h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
           ...
           <div className="glass p-6 rounded-xl border border-border/50 flex flex-col justify-between">
             <div>
               ...
               {compressedBlob && (
                 <img src={URL.createObjectURL(compressedBlob)} alt="Compressed" className="w-full h-auto rounded-lg mb-4" />
               )}
               <div className="flex justify-between items-center text-sm mb-8">
                 ...
                 {compressedBlob && (
                   <span className="font-mono bg-primary/20 text-primary px-2 py-1 rounded">
                     ...
                     <span className="text-xs ml-2 opacity-80">(-{Math.round((1 - compressedBlob.size / file.size) * 100)}%)</span>
                   </span>
                 )}
               </div>
