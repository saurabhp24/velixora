...
const POSITION_GRID: { value: Position; label: string }[] = [
            ...
            <div className="p-5 rounded-xl border border-white/10 bg-white/5 space-y-5">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {POSITION_GRID.map((pos) => (
                    <button key={pos.value} onClick={() => setPosition(pos.value)}
                      className={cn("py-2.5 rounded-xl border-2 text-xs font-medium transition-all duration-200", position === pos.value ? "border-primary bg-primary/10 text-primary" : "border-white/10 text-white/40 hover:border-white/25 hover:text-white/70")}>
                      {pos.label}
                    </button>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input type="number" value={startNum} onChange={(e) => setStartNum(e.target.value)} min="0" className="bg-background/50" />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Slider value={[fontSize]} onValueChange={([v]) => setFontSize(v)} min={8} max={24} step={1} />
                </div>
              </div>
            </div>
