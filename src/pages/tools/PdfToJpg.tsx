      ...
      <AnimatePresence mode="wait">
        {result ? (
          ...
          <motion.div key="editor" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-5">
            <FileInfoCard file={file} onRemove={handleReset} extra={<span className="text-xs text-muted-foreground">{isRendering ? "Rendering previews..." : `${pages.length} pages will become ${pages.length} JPG files`}</span>} />
                ...
                {pages.map((page, idx) => (
                  <div key={page.index} className="rounded-lg border border-white/10 bg-white/5 overflow-hidden">
                    <div className="aspect-[3/4] flex items-center justify-center">
                      {!page.thumbnail ? <Loader2 className="w-4 h-4 animate-spin text-white/20" /> : <img src={page.thumbnail} alt={`Page ${idx + 1}`} className="w-full h-full object-contain" draggable={false} />}
                    </div>
