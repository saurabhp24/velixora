...
export default function ResizeImage() {
  ...
  const handleProcess = async () => {
    ...
    if (isNaN(tw) || isNaN(th) || tw <= 0 || th <= 0) {
      toast({ title: "Error", description: "Enter valid dimensions.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      setResult({ blob, name: `resized-${tw}x${th}-${file.name.replace(`.${ext}`, `.${ext}`)}` });
            ...
            <div className="p-5 rounded-xl border border-white/10 bg-white/5 space-y-4">
              <div className="grid grid-cols-[1fr_auto_1fr] gap-3 items-end">
                <div className="space-y-1.5">
                  ...
                  <Input type="number" value={width} onChange={(e) => handleWidthChange(e.target.value)} className="bg-background/50" min="1" data-testid="width-input" />
                </div>
                <button
                  onClick={() => setLockAspect(!lockAspect)}
                  className="mb-1 p-2 rounded-lg hover:bg-white/10 transition-colors"
                ...
                <div className="space-y-1.5">
                  ...
                  <Input type="number" value={height} onChange={(e) => handleHeightChange(e.target.value)} className="bg-background/50" min="1" data-testid="height-input" />
                </div>
              </div>
              {naturalDims && (
                <p className="text-xs text-muted-foreground">Original: {naturalDims.w} × {naturalDims.h}px</p>
              )}
            </div>
