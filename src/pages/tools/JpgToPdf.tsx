...
interface ImageItem { id: string; file: File; src: string; }
    ...
    <Reorder.Item value={item} id={item.id} dragListener={false} dragControls={dragControls} whileDrag={{ scale: 1.05, zIndex: 50 }} layout transition={{ duration: 0.15 }}>
      <motion.div className="group relative rounded-xl border border-white/10 bg-white/5 overflow-hidden" whileHover={{ borderColor: "rgba(79,110,247,0.4)" }}>
        ...
        <div className="absolute top-1.5 right-1.5 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={onRemove} className="p-1 rounded-md bg-black/60 hover:bg-red-500/60 transition-colors"><X className="w-3 h-3 text-white" /></button>
          <div className="p-1 rounded-md bg-black/60 cursor-grab" onPointerDown={(e) => dragControls.start(e)} title="Drag to reorder">
...
export default function JpgToPdf() {
  ...
  const addFiles = useCallback((files: File[]) => {
    const newItems = files.map((f, i) => ({ id: `${f.name}-${Date.now()}-${i}`, file: f, src: URL.createObjectURL(f) }));
  ...
  useEffect(() => { return () => items.forEach((item) => URL.revokeObjectURL(item.src)); }, [items]);
            ...
            {items.length > 0 && (
              <div className="space-y-3">
                <div className="flex items-center justify-between px-1">
                  ...
                  <button onClick={handleReset} className="text-xs text-muted-foreground hover:text-foreground transition-colors">Clear all</button>
                </div>
                <Reorder.Group axis="y" values={items} onReorder={setItems} className="grid grid-cols-3 sm:grid-cols-4 gap-3" as="div">
                  {items.map((item) => <ImageCard key={item.id} item={item} onRemove={() => setItems((prev) => prev.filter((p) => p.id !== item.id))} />)}
                </Reorder.Group>
