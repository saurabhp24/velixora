...
export default function Convert() {
  ...
  const handleProcess = (f: File) => { setFiles(prev => [...prev, f]); };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto">
      <header className="mb-10 text-center">
        <h1 className="text-4xl font-bold tracking-tight mb-2">Convert & <span className="text-gradient">Merge</span></h1>
        <p className="text-muted-foreground">Convert images to PDF locally.</p>
      </header>
      <div className="mb-8"><FileDropZone onFileSelect={handleProcess} accept="image/*" title="Drop images to convert to PDF" /></div>
      {files.length > 0 && (
          ...
          <ul className="space-y-2 mb-6">
            {files.map((f, i) => (
              <li key={i} className="text-sm bg-background/50 px-3 py-2 rounded border border-border/50 flex justify-between">
                <span>{f.name}</span><span className="text-muted-foreground">{(f.size/1024).toFixed(1)} KB</span>
              </li>
            ))}
          </ul>
