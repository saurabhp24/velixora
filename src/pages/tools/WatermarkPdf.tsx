  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      for (const page of pdf.getPages()) {
        ...
        page.drawText(text, { x, y, size: fontSize, font, color: rgb(0.6, 0.6, 0.6), opacity: op, rotate: position === "center" ? degrees(45) : degrees(0) });
      }
          ...
          <motion.div key="editor" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-5">
            ...
            <div className="relative rounded-xl border border-white/10 bg-white/3 overflow-hidden" style={{ height: 120 }}>
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-4xl font-black tracking-widest select-none pointer-events-none" style={{ opacity: opacity / 100, color: "#999", position: "absolute",
            ...
            <div className="p-5 rounded-xl border border-white/10 bg-white/5 space-y-4">
              <div className="space-y-1.5">
                ...
                <Input value={watermark} onChange={(e) => setWatermark(e.target.value)} className="bg-background/50" placeholder="CONFIDENTIAL" />
              </div>
              <div className="space-y-2">
                ...
                <Slider value={[opacity]} onValueChange={([v]) => setOpacity(v)} min={5} max={80} step={5} />
              </div>
              <div className="space-y-1.5">
