  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto">
      <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-4xl font-bold tracking-tight mb-2">Smart <span className="text-gradient">OCR</span></h1>
          <p className="text-muted-foreground">Extract text from images locally. Fully private.</p>
        </div>
        <div className="w-48">
      ...
      {status === "idle" && <FileDropZone onFileSelect={handleProcess} accept="image/*" title="Drop an image to extract text" />}
      {status === "processing" && <AIProcessing status={progress || "Initializing OCR Engine..."} />}
      {status === "error" && (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => navigator.clipboard.writeText(result)}>Copy Text</Button>
              <Button onClick={() => {
                const blob = new Blob([result], { type: "text/plain" });
                const url = URL.createObjectURL(blob);
                const a = document.createElement("a"); a.href = url;
                a.download = `${file?.name ?? "ocr-result"}.txt`; a.click();
                URL.revokeObjectURL(url);
              }}>Download .txt</Button>
              <Button variant="ghost" onClick={() => { setStatus("idle"); setFile(null); }}>Extract Another</Button>
            </div>
          </div>
