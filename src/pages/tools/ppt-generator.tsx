  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-6xl mx-auto">
      <header className="mb-10 text-center">
        <h1 className="text-4xl font-bold tracking-tight mb-2">AI PPT <span className="text-gradient">Generator</span></h1>
      ...
      {status === "done" && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {slides.map((s, i) => <SlidePreview key={i} number={i + 1} title={s.title} points={s.points} />)}
        </div>
      )}
    </motion.div>
  );
