          ...
          <motion.div key="editor" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
            ...
            {files.length > 0 && (
              <div className="space-y-3">
                ...
                <div className="rounded-xl border border-white/10 bg-white/3 divide-y divide-white/5 overflow-hidden">
                  <AnimatePresence initial={false}>
                    {files.map((f, idx) => {
                      const { Icon, color } = getFileIcon(f.name);
                      return (
                        <motion.div key={`${f.name}-${idx}`} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="flex items-center gap-3 px-4 py-3">
                          ...
                          <button onClick={() => setFiles((prev) => prev.filter((_, i) => i !== idx))} className="p-1 rounded-lg hover:bg-white/10 text-white/30 hover:text-white/70 transition-colors flex-shrink-0">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    <Input value={archiveName} onChange={(e) => setArchiveName(e.target.value)} className="bg-background/50 flex-1" placeholder="archive" />
