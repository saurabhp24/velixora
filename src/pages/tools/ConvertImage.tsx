      ...
      <AnimatePresence mode="wait">
        {result ? (
          ...
          <motion.div key="editor" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-5">
            ...
            <div className="p-5 rounded-xl border border-white/10 bg-white/5 space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                {FORMATS.map((f) => {
                  ...
                  return (
                    <button key={f.value} onClick={() => setFormat(f.value)} disabled={isCurrent}
                      className={cn("relative flex flex-col items-center gap-1.5 p-4 rounded-xl border-2 transition-all duration-200", isSelected ? "border-primary bg-primary/10 text-primary" : "border-white/10 bg-white/3 text-white/50 hover:border-white/25 hover:text-white/80", isCurrent ? "opacity-40 cursor-not-allowed" : "cursor-pointer")}>
                      {isCurrent && <span className="absolute -top-2 left-1/2 -translate-x-1/2 text-[9px] bg-white/15 text-white/50 px-1.5 py-0.5 rounded-full">current</span>}
            ...
            <ProcessButton onClick={handleProcess} isProcessing={isProcessing} disabled={false} text={`Convert to ${FORMATS.find((f) => f.value === format)!.label}`} color={tool.color} />
          </motion.div>
        )}
      </AnimatePresence>
    </ToolPageLayout>
