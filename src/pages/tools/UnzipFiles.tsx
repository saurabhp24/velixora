...
interface ZipEntry { name: string; size: number; compressedSize: number; isDirectory: boolean; }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list: ZipEntry[] = [];
        zip.forEach((path, entry) => { list.push({ name: path, size: 0, compressedSize: 0, isDirectory: entry.dir }); });
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      for (const [path, entry] of Object.entries(zip.files)) {
        if (!entry.dir) { const blob = await entry.async("blob"); extracted.push({ name: path, blob }); }
      }
        ...
        {extractedFiles.length > 0 ? (
          <motion.div key="extracted" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" onClick={downloadAll} className="gap-1.5"><Download className="w-3.5 h-3.5" /> Download All</Button>
            </div>
            <div className="rounded-xl border border-white/10 divide-y divide-white/5 overflow-hidden max-h-96 overflow-y-auto">
              {extractedFiles.map(({ name, blob }, idx) => {
                ...
                return (
                  <motion.div key={idx} initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { delay: idx * 0.03 } }} className="flex items-center gap-3 px-4 py-3">
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: `${color}15`, color }}><Icon className="w-3.5 h-3.5" /></div>
                    ...
                    <Button size="sm" variant="ghost" onClick={() => saveAs(blob, name.split("/").pop() ?? name)} className="flex-shrink-0 h-7 px-2 text-xs"><Download className="w-3 h-3" /></Button>
                  </motion.div>
                );
              })}
            </div>
            <Button variant="outline" className="w-full" onClick={handleReset}>Extract Another ZIP</Button>
          </motion.div>
        ) : !file ? (
          ...
          <motion.div key="preview" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
            <FileInfoCard file={file} onRemove={handleReset} extra={isScanning ? <span className="text-xs text-muted-foreground">Scanning contents...</span> : <span className="text-xs text-muted-foreground">{entries.length} files inside</span>} />
            {entries.length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-xs text-muted-foreground px-1"><Package className="w-3.5 h-3.5" /><span>Archive contents</span></div>
                  ...
                  {visibleEntries.map((entry, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center gap-3 px-4 py-2.5">
                        <div className="w-6 h-6 rounded-md flex items-center justify-center flex-shrink-0" style={{ backgroundColor: `${color}15`, color }}><Icon className="w-3 h-3" /></div>
                ...
                {entries.length > 12 && !showAll && (
                  <button onClick={() => setShowAll(true)} className="w-full text-xs text-muted-foreground hover:text-foreground transition-colors flex items-center justify-center gap-1 py-2">
