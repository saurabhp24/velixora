            ...
            <div className="p-5 rounded-xl border border-white/10 bg-white/5 space-y-3">
              ...
              {isLoadingPreview ? (
                ...
                <div className="bg-black/20 rounded-lg p-4 max-h-48 overflow-y-auto">
                  <p className="text-sm text-foreground/60 leading-relaxed whitespace-pre-wrap font-mono">{textPreview}</p>
                </div>
              ) : null}
            </div>
            <ProcessButton onClick={handleProcess} isProcessing={isProcessing} disabled={isLoadingPreview} text="Extract to Text File" color={tool.color} />
          </motion.div>
        )}
      </AnimatePresence>
    </ToolPageLayout>
