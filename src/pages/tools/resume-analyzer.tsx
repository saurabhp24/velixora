  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto">
      <header className="mb-10 text-center">
        <h1 className="text-4xl font-bold tracking-tight mb-2">Resume <span className="text-gradient">Analyzer</span></h1>
      </header>
          ...
          <div className="col-span-2 space-y-6">
            ...
            <div className="glass p-6 rounded-xl border border-border/50">
              ...
              <div className="flex flex-wrap gap-2">
                {analysis.keywords?.map((k: string, i: number) => <span key={i} className="bg-cyan-500/10 text-cyan-400 px-3 py-1 rounded-full text-xs">{k}</span>)}
              </div>
            </div>
          </div>
        </div>
