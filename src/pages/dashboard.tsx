  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-6xl mx-auto">
      <header className="mb-10">
        <h1 className="text-4xl font-bold tracking-tight mb-2">Workspace <span className="text-gradient">Dashboard</span></h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <section>
            ...
            {files.length === 0 ? (
              <div className="glass rounded-xl p-10 text-center border border-border/50">
                ...
                <p className="text-muted-foreground text-sm mb-6">Use a tool to get started.</p>
                ...
                {files.map((file, i) => (
                  <div key={i} className="glass p-4 rounded-xl border border-border/50 flex items-center justify-between group hover:border-primary/30 transition-colors">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-lg bg-primary/10 text-primary flex items-center justify-center shrink-0"><FileIcon className="w-5 h-5" /></div>
                      <div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="glass p-6 rounded-xl border border-border/50">
                ...
                <Link href="/tools/contract-analyzer" className="text-sm text-primary hover:underline">Start Workflow →</Link>
              </div>
              <div className="glass p-6 rounded-xl border border-border/50">
                ...
                <Link href="/tools/resume-analyzer" className="text-sm text-primary hover:underline">Start Workflow →</Link>
              </div>
            </div>
          </section>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-6 glass rounded-xl border border-border/50 overflow-hidden flex flex-col h-[500px]">
            <div className="p-4 border-b border-border/50 bg-background/50 backdrop-blur-md">
              <h3 className="font-semibold flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-cyan-400 animate-pulse" /> AI Assistant
              </h3>
            </div>
            <div className="flex-1 p-4 flex flex-col items-center justify-center text-center">
              <BrainCircuit className="w-12 h-12 text-muted-foreground/20 mb-4" />
              <p className="text-sm text-muted-foreground px-6">Open a tool to interact with the AI. Your context will appear here.</p>
            </div>
          </div>
        </div>
      </div>
