  ...
  return (
    ...
    >
      ...
      <div className="bg-background/50 border border-border/50 rounded-lg py-3 px-6 mb-8 flex items-center gap-4">
        ...
        {size && <span className="text-sm text-muted-foreground bg-white/5 px-2 py-1 rounded">{formatSize(size)}</span>}
      </div>
      <div className="flex flex-col sm:flex-row items-center gap-4 w-full max-w-md">
        <Button size="lg" onClick={onDownload} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground shadow-[0_0_20px_rgba(79,110,247,0.3)] hover:shadow-[0_0_30px_rgba(79,110,247,0.5)] transition-all duration-300">
          <Download className="w-5 h-5 mr-2" /> Download File
        </Button>
        <Button size="lg" variant="outline" onClick={onReset} className="w-full border-border/50 hover:bg-white/5">
          <RefreshCw className="w-5 h-5 mr-2" /> Process Another
        </Button>
      </div>
    </motion.div>
  );
