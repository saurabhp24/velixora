    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center justify-center py-20 px-6 text-center h-full min-h-[300px]">
        <AnimatePresence mode="wait">
          {isProcessing ? (
            <motion.div key="processing" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} className="flex flex-col items-center">
              <Loader2 className="w-16 h-16 text-primary animate-spin mb-4" />
              <p className="text-lg font-medium text-foreground">Processing Files...</p>
            </motion.div>
          ) : isDragActive ? (
            <motion.div key="active" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} className="flex flex-col items-center text-primary">
              <CheckCircle2 className="w-16 h-16 mb-4" />
              <p className="text-xl font-bold">Drop files here!</p>
            </motion.div>
          ) : isDragReject ? (
            <motion.div key="reject" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} className="flex flex-col items-center text-destructive">
              <FileType className="w-16 h-16 mb-4" />
              <p className="text-xl font-bold">Unsupported file type</p>
            </motion.div>
          ) : (
            <motion.div key="idle" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} className="flex flex-col items-center">
              <div
              ...
              <p className="text-sm text-muted-foreground mt-2 max-w-md">
                {tool
                  ? `Upload your file to begin processing. Everything happens securely in your browser.`
                  : "Upload your files to begin processing."}
              </p>
            </motion.div>
          )}
        </AnimatePresence>
